<?php
	...
	$user = list_discipline($_SESSION['id_cont_prof']);
	if($user == FALSE){
		redirect('error');
		exit();
	}
...
<head>
			<!-- BARA DE TITLU-->
	<title>Editeaza disciplina</title>
...
<body>
	...
	?>
	<br><br><br>
	<form action="<?php echo BASE_URL;?>index.php/edit_discipline" method="POST">
		<input type="hidden" name="id" value="<?php echo $_GET['id'];?>">
		<label for="nume_disciplina" class="eticheta_formular">Numele disciplinei:</label>
		<?php
			include_once "model/functii_select.php";
			$result = get_discipline($_GET['id']);
			echo "
				<input type='text' name='nume_disciplina' id='input_intro' value='".$result[0]['Nume_disciplina']."' required>
			";
		?>
		<br>
		<div class="clear">
